...
namespace NBADWDataEntryApplication
{
	...
	public partial class Login : System.Web.UI.Page
	{
		...
		protected System.Data.DataView dvUser;
		...
		{    
			...
			this.dvUser = new System.Data.DataView();
			...
			// oleDbInsertCommand1
			// 
			this.oleDbInsertCommand1.CommandText = "INSERT INTO [User] (Administrator, FirstName, LastLogin, LastName, [Password], Us" +
				"erDatabase, UserName, UserAgency, Organization) VALUES (?, ?, ?, ?, ?, ?, ?, ?, " +
				"?)";
			...
			// oleDbSelectCommand1
			// 
			this.oleDbSelectCommand1.CommandText = "SELECT Administrator, FirstName, LastLogin, LastName, [Password], UserDatabase, U" +
				"serID, UserName, UserAgency, Organization FROM [User]";
			this.oleDbSelectCommand1.Connection = this.oleDbConnection1;
			// 
			// oleDbUpdateCommand1
			// 
			this.oleDbUpdateCommand1.CommandText = @"UPDATE [User] SET Administrator = ?, FirstName = ?, LastLogin = ?, LastName = ?, [Password] = ?, UserDatabase = ?, UserName = ?, UserAgency = ?, Organization = ? WHERE (UserID = ?) AND (Administrator = ?) AND (FirstName = ? OR ? IS NULL AND FirstName IS NULL) AND (LastLogin = ? OR ? IS NULL AND LastLogin IS NULL) AND (LastName = ? OR ? IS NULL AND LastName IS NULL) AND (Organization = ? OR ? IS NULL AND Organization IS NULL) AND ([Password] = ? OR ? IS NULL AND [Password] IS NULL) AND (UserAgency = ? OR ? IS NULL AND UserAgency IS NULL) AND (UserDatabase = ? OR ? IS NULL AND UserDatabase IS NULL) AND (UserName = ? OR ? IS NULL AND UserName IS NULL)";
			...
			// dvUser
			// 
			this.dvUser.Sort = "UserName";
			this.dvUser.Table = this.objdsUser.User;
		...
		{
			LoadUsers();
			int i = dvUser.Find(txtUserName.Text);
			if(i>=0)//user found
			{
				if(dvUser[i]["Password"].ToString()==txtPassword.Text)//password is valid
				{
					Session["Administrator"] = dvUser[i]["Administrator"];
					Session["UserAgency"] = dvUser[i]["UserAgency"];
					string strConn = dvUser[i]["UserDatabase"].ToString();;
					Session["ConnectionString"] = @"Jet OLEDB:Global Partial Bulk Ops=2;Jet OLEDB:Registry Path=;Jet OLEDB:Database Locking Mode=1;Data Source="""+strConn+@""";Jet OLEDB:Engine Type=5;Provider=""Microsoft.Jet.OLEDB.4.0"";Jet OLEDB:System database=;Jet OLEDB:SFP=False;persist security info=False;Extended Properties=;Mode=Share Deny None;Jet OLEDB:Encrypt Database=False;Jet OLEDB:Create System Database=False;Jet OLEDB:Don't Copy Locale on Compact=False;Jet OLEDB:Compact Without Replica Repair=False;User ID=Admin;Jet OLEDB:Global Bulk Transactions=1";
                    dvUser[i]["LastLogin"] = System.DateTime.Now;
					try
					{
						//this.oleDbdaUser.Update(objdsUser);
						string sql = "UPDATE [User] SET [User].LastLogin = Now() WHERE ((User.UserName) = '"+txtUserName.Text+"')";
						Debug.WriteLine("String: "+sql);
					...
					{
						oleDbConnection1.Close();
						Debug.WriteLine("Error in updating user database: "+ex.ToString());
					}
					Server.Transfer("MainMenu.aspx");
				}
				else
				{
					lblMessage.Text = "Invalid password.  Please retry.";
			...
			{
				lblMessage.Text = "Invalid user name.  Please retry.";
				lblMessage.Visible = true;
				txtPassword.Text = "";
			}
		}
